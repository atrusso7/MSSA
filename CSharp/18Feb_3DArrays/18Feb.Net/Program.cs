...
namespace _18Feb.Net
{
    class Program
    {
        static void GCD(int a, int b)
        {
            ...
            List<int> bDiv = new List<int>();
            List<int> cDiv = new List<int>();
            while (a >= i)
            {
                int result = a % i;
                if(result == 0)
            ...
            for (int o = 0; o < aDiv.Count; o++)
            {
                int number = aDiv[o];  // Note the use of array syntax
                bool there = bDiv.Contains(aDiv[o]);
                if (there == true)
                {
                    cDiv.Add(number);
                }
            ...
            max = cDiv.Max();
            Console.WriteLine("The GCD is: " + max);
        }
